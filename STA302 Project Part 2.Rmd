...
coef_table <- tidy(preferred_train) %>%
  mutate(
...
write_csv(coef_table, "figures/coef_table_preferred_model.csv")

# Optional: coefficient plot (dot-and-whisker style)
coef_plot <- coef_table %>%
  ...
  labs(title = "Coefficient Estimates with 95% CI", x = "Estimate", y = "") +
  theme_minimal()

ggsave("figures/coef_plot_preferred_model.png", coef_plot, width = 7, height = 4, dpi = 300)
```
